...
export async function POST(req: NextRequest) {
  ...
  if (!b.voterId) return NextResponse.json({ error: "selecione um eleitor" }, { status: 400 });
  ...
  const [row] = await db.insert(demands).values({
    ...
    voterId: v.id,
    coordinatorId: v.coordinatorId ?? coordinatorScopeIdForUser(s),
    createdBy: s.id,
  }).returning({ id: demands.id });
  await audit({ actorId: s.id, actorRole: s.role, action: "demand_create", entity: "demands", entityId: row.id, detail: `Eleitor #${v.id}: ${b.title}`, ip: ipOf(req) });

...
export default function CoordinatorsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="card">
          <h3 className="text-base font-bold text-brand-blue mb-4 flex items-center gap-2">
            <span className="w-1 h-5 rounded-full bg-brand-orange inline-block" /> Cadastrar Coordenador
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-xs font-semibold text-brand-blue/60 uppercase tracking-wider mb-1.5">Nome *</label>
                <input className="input-field" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required /></div>
              <div><label className="block text-xs font-semibold text-brand-blue/60 uppercase tracking-wider mb-1.5">E-mail *</label>
                <input type="email" className="input-field" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required /></div>
              <div><label className="block text-xs font-semibold text-brand-blue/60 uppercase tracking-wider mb-1.5">Senha *</label>
      ...
      <div className="card"><input className="input-field" placeholder="🔍 Buscar coordenador..." value={search} onChange={(e) => setSearch(e.target.value)} /></div>
      ...
      <div className="card overflow-hidden p-0">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-50">
            ...
            : coordinators.length === 0 ? <tr><td colSpan={7} className="table-cell text-center text-gray-300">Nenhum coordenador</td></tr>
            : coordinators.map((u) => (

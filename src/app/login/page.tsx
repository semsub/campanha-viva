...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Faz o fetch e trata QUALQUER resposta que não seja JSON válido
    // (útil quando o Render está suspenso e devolve HTML "Service Suspended")
    fetch("/api/bootstrap")
      .then(async (r) => {
        ...
        try {
          ...
          // Resposta veio como HTML/texto — provavelmente Render suspenso ou 503
          throw new Error(
            r.status === 503
              ? "Servidor suspenso. Reative o serviço no painel do Render."
              : `Resposta inválida do servidor (HTTP ${r.status}). O serviço pode estar suspenso.`,
          );
        }
      })
      .then(setStatus)
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try { data = JSON.parse(text); } catch {
        setError(
          res.status === 503
            ? "Servidor suspenso. Reative no painel do Render."
